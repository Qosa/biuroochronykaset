...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'%s - zarejestrowano pomyślnie!' % form.name.data, 'success')
        login_user(the_user)
        return redirect(request.args.get('next') or url_for('main.index'))
    return render_template('register.html', form=form, title=u"Rejestracja nowego użytkownika")
...
def change_password():
    ...
    return render_template('user_edit.html', form=form, user=current_user, title=u"Zmień hasło")
